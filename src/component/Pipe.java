 ...
 */
public class Pipe {
    static BufferedImage[] imgs; // 水管的图片, static 保证图片只加载一次
    ...
    boolean visible; // 水管可见状态, true 为可见, false 表示可归还至对象池
    // 水管的类型
    int type;
    ...
    private void drawTopNormal(Graphics g) {
        ...
        g.drawImage(imgs[1], x - ((PIPE_HEAD_WIDTH - width) >> 1),
                height - Constant.TOP_PIPE_LENGTHENING - PIPE_HEAD_HEIGHT, null); // 水管头部与水管主体的宽度不同, x 坐标需要处理
    }
    ...
    /**
     * 判断当前水管是否完全出现在窗口中
     * @return 若完全出现则返回 true, 否则返回 false
     */
