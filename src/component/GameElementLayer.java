...
public class GameElementLayer {
    ...
    /**
     * 添加水管的逻辑: 当容器中添加的最后一个元素完全显示到屏幕后, 添加下一对; 水管成对地相对地出现, 空隙高度为窗口高度的 1/6;
     * 每对水管的间隔距离为屏幕高度的 1/4; 水管的高度的取值范围为窗口的 [1/8 ~ 5/8]
     */
        ...
        if (pipes.size() == 0) {
            ...
            if (lastPipe.isInFrame()) {
                ...
                try {
                    ...
                    // 移动水管刷新的概率随当前分数递增, 当得分大于 19 后全部刷新移动水管
    ...
    /**
     * 添加普通水管
     * @param lastPipe 传入最后一根水管以获取 x 坐标
     */
    private void addNormalPipe(Pipe lastPipe) {
        int topHeight = GameUtil.getRandomNumber(MIN_HEIGHT, MAX_HEIGHT + 1); // 随机生成水管高度
        int x = lastPipe.getX() + HORIZONTAL_INTERVAL; // 新水管的 x 坐标 = 最后一对水管的 x 坐标 + 水管的间隔

        Pipe top = PipePool.get("Pipe"); // 从水管对象池中获取对象

        // 设置 x, y, height, type 属性
    ...
    private void addHoverPipe(Pipe lastPipe) {
        ...
        int x = lastPipe.getX() + HORIZONTAL_INTERVAL; // 新水管的 x 坐标 = 最后一对水管的 x 坐标 + 水管的间隔
        int y = GameUtil.getRandomNumber(Constant.FRAME_HEIGHT / 12, Constant.FRAME_HEIGHT / 6); // 随机水管的 y 坐标, 窗口的 [1/6, 1/12]
